...
- codeExamples: Array<{pattern: string, code: string, explanation: string}>`;
        ...
        return Effect.tryPromise({
          ...
          Effect.tapError((error) =>
            Effect.logError(
              `Chunk analysis failed after retries: ${error._tag} - ${"message" in error ? error.message : "Unknown error"
              }`,
            ),
          ),
        );
...
Format the output as well-structured Markdown.`;
        ...
        return Effect.tryPromise({
          ...
          Effect.tapError((error) =>
            Effect.logError(
              `Analysis aggregation failed after retries: ${error._tag} - ${"message" in error ? error.message : "Unknown error"
              }`,
            ),
          ),
        );
