...
export function TaskChat({ taskId, task }: TaskChatProps) {
  ...
  // Auto-refresh disabled to avoid background API requests/logs
  useEffect(() => {
  ...
  const renderTabContent = () => {
    ...
    const agentMessages = displayMessages.filter((message) => message.role === 'agent')
    ...
    const getMessageWrapperClass = (isUserMessage: boolean, index: number) => {
      const baseClass = index > 0 ? 'mt-4' : ''
      const stickyClass = isUserMessage
        ? "sticky top-0 z-10 before:content-[''] before:absolute before:inset-0 before:bg-background before:-z-10"
        : ''

      return `${baseClass} ${stickyClass}`.trim()
    }

    const addShimmerMarker = (content: string, isLastAgentMessage: boolean) => {
      ...
      const lastIndex = lastMatch?.index !== undefined ? lastMatch.index + 2 : 0
    ...
    return (
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto pb-4">
        ...
        {displayMessages.map((message, index) => {
          ...
          const wrapperClass = getMessageWrapperClass(isUserMessage, index)
          ...
          const isLastAgentMessage = lastAgentMessageId === message.id
          const parsedContent = parseAgentMessage(message.content)
          const contentWithShimmer = addShimmerMarker(parsedContent, isLastAgentMessage)

          return (
            <div key={message.id} className={wrapperClass}>
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground px-2">
                  {!message.content.trim() && isAgentWorking ? (
                    (() => {
                    ...
                    <Streamdown
                      components={{
                        ...
                        p: ({ children, ...props }: React.ComponentPropsWithoutRef<'p'>) => {
                          ...
                          const isToolCall = /^(🔄SHIMMER🔄)?(Editing|Reading|Running|Listing|Executing)/i.test(text)

                          const displayText = text.replace('🔄SHIMMER🔄', '')
                          const hasReactElements = childrenArray.some((child) => isValidElement(child))
                          const contentToRender = hasReactElements ? childrenArray : displayText
