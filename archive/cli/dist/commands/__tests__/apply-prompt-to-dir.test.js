...
This is a {{ style }} guide about {{ topic }}.`;
        ...
        }).pipe(Effect.provide(Layer.mergeAll(NodeFileSystem.layer, NodePath.layer, TemplateService.Default)));
    });
...
Content with {{ required_param }}.`;
        return Effect.gen(function* () {
            ...
            const result = yield* templateService.renderTemplate(template, {}).pipe(Effect.flip);
