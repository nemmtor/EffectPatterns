...
export const configSet = Command.make("set", {
    ...
    if (asJson) {
        yield* printJson({ set: { key, value } }, getGlobalCompact(), outputFile ? { outputFile } : undefined);
    }
    else {
        yield* printText(`Set ${key}=${value}`, outputFile ? { outputFile } : undefined);
    }
}));
