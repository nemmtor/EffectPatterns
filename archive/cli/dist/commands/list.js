...
export const effectPatternsList = makeCommand("list", { pathspec, verbose, json, quiet, noColor, output, force }, ({ pathspec, verbose, json, quiet, noColor, output, force }) => {
    return Effect.gen(function* () {
        ...
        if (asJson) {
            ...
            yield* printJson(jsonPayload, getGlobalCompact(), resolvedOutput ? { outputFile: resolvedOutput } : undefined);
        }
        else if (!quietFlag) {
            const content = files.map((f) => `  ${f}`).join("\n");
            yield* printText(content, resolvedOutput ? { outputFile: resolvedOutput } : undefined);
        }
