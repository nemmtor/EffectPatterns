...
export const metricsLast = Command.make("last", {
    ...
    if (last._tag === "None") {
        if (asJson) {
            yield* printJson({ last: null }, getGlobalCompact(), outputFile ? { outputFile } : undefined);
        }
        else {
            yield* printText("[METRICS] No metrics data available", outputFile ? { outputFile } : undefined);
        }
        return;
    }
    const run = last.value;
    if (asJson) {
        yield* printJson({ last: run }, getGlobalCompact(), outputFile ? { outputFile } : undefined);
        return;
    }
