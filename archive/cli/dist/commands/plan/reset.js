...
export const planReset = Command.make("reset", { output: Options.text("output").pipe(Options.optional, Options.withAlias("o")) }, (opts) => Effect.gen(function* () {
    ...
    if (asJson) {
        yield* printJson({ reset: true }, getGlobalCompact(), outputFile ? { outputFile } : undefined);
    }
    else {
        yield* printText("Reset to default execution plan (retries=1, retryMs=1000)", outputFile ? { outputFile } : undefined);
    }
}));
