...
export const runCurrent = Command.make("current", {
    json: Options.boolean("json").pipe(Options.optional, Options.withDescription("Output machine-readable JSON")),
    output: Options.text("output").pipe(Options.optional, Options.withAlias("o"), Options.withDescription("Write output to file")),
}, ({ json, output }) => Effect.gen(function* () {
    ...
    const outFile = Option.getOrElse(output, () => undefined);
    const outOptions = outFile ? { outputFile: outFile, force: true } : undefined;
    if (!current) {
        if (asJson) {
            yield* printJson({ current: null }, getGlobalCompact(), outOptions);
        }
        else {
    ...
    if (asJson) {
        yield* printJson({ current }, getGlobalCompact(), outOptions);
    }
    else {
