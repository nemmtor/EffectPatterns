...
export const run = makeCommandGroup("run", {
    json: Options.boolean("json").pipe(Options.optional, Options.withDescription("Output JSON when supported by a subcommand")),
    compact: Options.boolean("compact").pipe(Options.optional, Options.withDescription("Compact JSON output (no pretty-print)")),
}, [runCreate, runCurrent, runPath, runList, runUse, runDelete], {
    description: "Manage runs (create, use, list, path, current, delete)",
    onInit: ({ json, compact }) => {
        const globalJson = Option.getOrElse(json, () => false);
        const globalCompact = Option.getOrElse(compact, () => false);
