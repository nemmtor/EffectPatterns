...
export function createCli(options) {
    ...
    const otelServiceName = Options.text("otel-service-name").pipe(Options.optional, Options.withDescription("OTel service name (default: effect-ai-cli or $OTEL_SERVICE_NAME)"));
    const rootBase = Command.make(options.name ?? "effect-ai", { configs, version, help, run, otel, otelEndpoint, otelServiceName }, () => Effect.void);
    const all = [
        ...commands,
