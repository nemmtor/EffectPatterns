 ...
 */
const MinimalPlatformLayer = Layer.mergeAll(NodeContext.layer, NodeHttpClient.layer, NodeFileSystem.layer, NodePath.layer, NodeTerminal.layer);
/**
 ...
 */
const MinimalAppLayer = Layer.mergeAll(ConfigService.Default, MetricsService.Default, MdxService.Default, TemplateService.Default, OtelService.Default, OutputHandlerService.Default, RunService.Default);
/**
 * Live environment layer with minimal configuration.
 */
const MinimalLiveEnv = Layer.setConfigProvider(MinimalConfigProvider).pipe(Layer.provide(MinimalPlatformLayer));
/**
 ...
 */
export const MinimalLayers = Layer.provide(MinimalAppLayer, MinimalLiveEnv).pipe(Layer.merge(MinimalLiveEnv));
/**
 ...
 * ```
 */
export const MinimalRuntime = ManagedRuntime.make(Layer.mergeAll(Layer.provideMerge(MinimalAppLayer, MinimalPlatformLayer), Layer.setConfigProvider(MinimalConfigProvider)));
/**
 * Helper function to run effects in the minimal runtime.
 */
export const runMinimalEffect = (effect) => MinimalRuntime.runPromise(effect);
/**
 * Helper function to run effects and get the Exit result in minimal runtime.
 */
export const runMinimalExit = (effect) => MinimalRuntime.runPromiseExit(effect);
//# sourceMappingURL=minimal-runtime.js.map