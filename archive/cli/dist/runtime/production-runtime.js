 ...
 */
const ProductionPlatformLayer = Layer.mergeAll(NodeContext.layer, NodeHttpClient.layer, NodeFileSystem.layer, NodePath.layer, NodeTerminal.layer);
/**
...
const AiClientLayers = Layer.mergeAll(Layer.scoped(GoogleAiClient.GoogleAiClient, Effect.flatMap(Config.redacted("GOOGLE_AI_API_KEY"), (apiKey) => GoogleAiClient.make({ apiKey }))), Layer.scoped(OpenAiClient.OpenAiClient, Effect.flatMap(Config.redacted("OPENAI_API_KEY"), (apiKey) => OpenAiClient.make({ apiKey }))), Layer.scoped(AnthropicClient.AnthropicClient, Effect.flatMap(Config.redacted("ANTHROPIC_API_KEY"), (apiKey) => AnthropicClient.make({ apiKey }))));
const AppLayer = Layer.mergeAll(ConfigService.Default, AuthService.Default, MetricsService.Default, MdxService.Default, OtelService.Default, OutputHandlerService.Default, RunService.Default, LLMService.Default, TemplateService.Default, AiClientLayers);
/**
...
const LiveEnv = Layer.merge(ProductionPlatformLayer, Layer.setConfigProvider(ProductionConfigProvider));
/**
...
export const ProductionLayers = Layer.provide(AppAndAiLayer, LiveEnv).pipe(Layer.merge(LiveEnv));
/**
...
export const ProductionRuntime = ManagedRuntime.make(Layer.mergeAll(Layer.provideMerge(AppLayer, ProductionPlatformLayer), Layer.setConfigProvider(ProductionConfigProvider)));
/**
...
export const runProductionEffect = (effect) => {
...
export const runProductionExit = (effect) => {
