...
describe("Stage 6: Output File Redirection", () => {
    ...
    it("should handle file writing successfully", async () => {
        ...
        const writeEffect = Effect.scoped(Effect.gen(function* () {
    ...
    it("should handle file writing errors gracefully", async () => {
        ...
        const result = await Effect.runPromise(Effect.cause(writeEffect).pipe(Effect.provide(testLayer), Effect.map(cause => cause._tag)));
