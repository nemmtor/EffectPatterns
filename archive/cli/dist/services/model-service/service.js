...
export class ModelService extends Effect.Service()("ModelService", {
    sync: () => ({
        ...
        getModels: (providerName) => {
            ...
            if (provider) {
                const models = hardcodedModels.filter((m) => provider.supportedModels.includes(m.name));
        ...
        getModelsByCapability: (capability) => Effect.succeed(hardcodedModels.filter((m) => m.capabilities.includes(capability))),
