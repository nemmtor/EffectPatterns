...
describe("ApplyPromptToDirCommand - Final Fixed Tests", () => {
    it("should process files correctly with platform services", async () => {
        const result = await runTestEffect(Effect.gen(function* () {
            // Access FileSystem and Path services correctly
            const fs = yield* FileSystem.FileSystem;
            const path = yield* Path.Path;
            const testDir = "/tmp/test-apply-prompt-final";
