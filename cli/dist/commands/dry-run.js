...
const estimateTokensAndCost = (provider, model, prompt) => Effect.gen(function* () {
...
export const dryRun = Command.make("dry-run", {
    provider: Options.choice("provider", ["openai", "anthropic", "google"]).pipe(Options.withDescription("LLM provider to estimate for")),
    model: Options.text("model").pipe(Options.withDescription("Model to estimate for")),
    prompt: Args.text({ name: "prompt" }).pipe(Args.optional),
    file: Options.file("file").pipe(Options.optional, Options.withDescription("Read prompt from file")),
