...
const runCreate = Command.make("create", {
    prefix: Options.text("prefix").pipe(Options.optional, Options.withAlias("p")),
    quiet: Options.boolean("quiet").pipe(Options.optional, Options.withAlias("q")),
}, ({ prefix, quiet }) => Effect.gen(function* () {
...
const runCurrent = Command.make("current", {}, () => Effect.gen(function* () {
    const runService = yield* RunService;
...
const runPath = Command.make("path", {}, () => Effect.gen(function* () {
    const runService = yield* RunService;
...
export const runCommand = Command.make("run").pipe(Command.withSubcommands([runCreate, runCurrent, runPath, runList]));
// Add: run use <runName>
export const runUse = Command.make("use", { name: Options.text("name") }, ({ name }) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const runService = yield* RunService;
    const runsDir = path.join(process.cwd(), "runs");
    ...
    if (!(runDir === normalizedRuns || runDir.startsWith(`${normalizedRuns}/`))) {
...
export const run = Command.make("run").pipe(Command.withSubcommands([
    runCreate,
    runCurrent,
    runPath,
    runList,
