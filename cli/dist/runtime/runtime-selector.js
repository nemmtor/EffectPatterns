...
export function selectRuntime(args) {
    ...
    const commandIndex = args.findIndex((arg, index) => index >= 2 &&
        !arg.startsWith("-") &&
 ...
 */
export function runWithAppropriateRuntime(effect, args = process.argv) {
 ...
 */
export function runExitWithAppropriateRuntime(effect, args = process.argv) {
    const runtime = selectRuntime(args);
