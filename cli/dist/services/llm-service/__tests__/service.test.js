...
import { generateObject, generateText, processPromptFromMdx, processPromptFromText, streamText, } from "../service.js";
...
const TestAiClientLayers = Layer.mergeAll(GoogleAiClient.layerConfig({
...
const TestLayer = Layer.mergeAll(NodeContext.layer, NodeFileSystem.layer, NodeHttpClient.layer, NodePath.layer, TestAiClientLayers, Layer.setConfigProvider(ConfigProvider.fromEnv()));
describe("LLM Service - Comprehensive Testing Suite", () => {
    ...
    describe("Basic Function Execution", () => {
        test("generateText should return valid Effect that can be executed", () => Effect.gen(function* () {
        ...
        test("generateObject should return valid Effect that can be executed", () => Effect.gen(function* () {
            const effect = generateObject(testPrompt, testSchema, "google", "gemini-2.5-flash");
            const result = yield* effect;
            expect(result).toBeDefined();
        ...
        test("streamText should return valid Stream that can be executed", () => Effect.gen(function* () {
            ...
            const text = yield* Stream.runFold(stream, "", (acc, chunk) => {
                ...
                return acc + chunk.text || JSON.stringify(chunk);
            });
            expect(text.length).toBeGreaterThan(0);
        }).pipe(Effect.provide(TestLayer)));
        test("processPromptFromText should return valid Effect that can be executed", () => Effect.gen(function* () {
        ...
        test("processPromptFromMdx should return valid Effect that can be executed", () => Effect.gen(function* () {
