...
export const readFileContent = (filePath) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const content = yield* fs.readFileString(filePath);
    return content;
}).pipe(Effect.catchAll((error) => Effect.fail(new FileReadError({ filePath, reason: error instanceof Error ? error.message : String(error) }))));
