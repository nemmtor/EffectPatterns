...
export class MdxService extends Effect.Service()("MdxService", {
    effect: Effect.gen(function* () {
        ...
        const readMdxAndFrontmatter = (filePath) => fs.readFileString(filePath).pipe(Effect.flatMap((content) => 
        ...
        Effect.sync(() => {
            ...
            if (parts.length < 3) {
                throw new InvalidMdxFormatError({
                    reason: "Missing or malformed frontmatter block (expected '---' delimiters).",
                });
            }
        ...
        Effect.mapError((e) => new InvalidMdxFormatError({
            reason: `Failed to parse frontmatter in ${filePath.toString()}: ${String(e)}`,
        })))));
        // Reconstruct MDX content with updated frontmatter
        const updateMdxContent = (originalFullMdxContent, updatedFrontmatter) => {
            ...
            if (parts.length >= 3) {
                ...
                const secondDelimiterEndIndex = originalFullMdxContent.indexOf("---", originalFullMdxContent.indexOf("---") + 3) + 3;
                originalBodyContent = originalFullMdxContent.substring(secondDelimiterEndIndex);
            }
            else {
        ...
        const parseMdxFile = (content) => Effect.gen(function* () {
            // Extract frontmatter
            const frontmatterMatch = content.match(/^---\s*\n([\s\S]*?)\n---\s*\n([\s\S]*)$/);
            if (!frontmatterMatch) {
                return yield* Effect.fail(new InvalidMdxFormatError({
            ...
            const metadataResult = yield* Effect.try({
                try: () => parseYaml(frontmatterContent),
                catch: (error) => new InvalidMdxFormatError({
        ...
        }).pipe(Effect.catchAll((error) => Effect.fail(new InvalidMdxFormatError({
            reason: error instanceof Error ? error.message : "Failed to parse MDX",
        }))));
        // Validate MDX configuration
        const validateMdxConfig = (attributes) => {
            // Extract common fields that might be used across services
            const provider = typeof attributes.provider === "string"
                ? attributes.provider
                : undefined;
            const model = typeof attributes.model === "string" ? attributes.model : undefined;
            const parameters = typeof attributes.parameters === "object" &&
        ...
        const extractParameters = (metadata) => {
            const parameters = {};
            ...
            for (const [key, value] of Object.entries(paramsObj)) {
                if (typeof value === "object" && value !== null && "type" in value) {
                    ...
                    if (typeof type === "string" &&
                        ...
                        parameters[key] = {
                            type: type,
                            description: typeof paramValue.description === "string"
                                ? paramValue.description
                                : undefined,
                            required: typeof paramValue.required === "boolean"
                                ? paramValue.required
                                : undefined,
