...
const estimateCost = (provider, model, totalTokens) => {
    ...
    const providerCosts = costPerThousand[provider];
    const costPerToken = providerCosts ? providerCosts[model] || 0.001 : 0.001;
...
describe("MetricsService", () => {
    ...
    it("should track command lifecycle", async () => {
        const optionResult = await Effect.runPromise(Effect.gen(function* () {
            const metrics = yield* MetricsService;
