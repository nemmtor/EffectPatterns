...
import { ModelService, make, } from "../service.js";
describe("ModelService", () => {
    const runEffect = (effect) => Effect.runPromiseExit(effect.pipe(Effect.provideService(ModelService, make)));
    describe("getModel", () => {
        ...
        it("should fail with ModelNotFoundError for non-existent model", async () => {
            ...
            if (Exit.isFailure(exit)) {
                ...
                expect(cause.error._tag).toBe("ModelNotFoundError");
                expect(cause.error.modelName).toBe("non-existent-model");
            }
        });
        it("should fail with ModelNotFoundError for empty model name", async () => {
            ...
            if (Exit.isFailure(exit)) {
                ...
                expect(cause._tag).toBe("Fail");
                expect(cause.error._tag).toBe("ModelNotFoundError");
                expect(cause.error.modelName).toBe("");
            }
        });
        it("should fail with ModelNotFoundError for case-sensitive model names", async () => {
            ...
            if (Exit.isFailure(exit)) {
                ...
                expect(cause._tag).toBe("Fail");
                expect(cause.error._tag).toBe("ModelNotFoundError");
                expect(cause.error.modelName).toBe("GPT-4");
            }
        });
    });
    describe("getModels", () => {
        it("should retrieve all models for OpenAI provider", async () => {
            ...
            if (Exit.isSuccess(exit)) {
                ...
                expect(models.every((model) => model.capabilities.includes("text-generation") ||
                    model.capabilities.includes("code-generation"))).toBe(true);
            }
        });
        ...
        it("should fail with ProviderNotFoundError for non-existent provider", async () => {
            ...
            if (Exit.isFailure(exit)) {
                const cause = exit.cause;
                expect(cause._tag).toBe("Fail");
                expect(cause.error._tag).toBe("ProviderNotFoundError");
                expect(cause.error.providerName).toBe("NonExistentProvider");
            }
        });
        it("should fail with ProviderNotFoundError for empty provider name", async () => {
            ...
            if (Exit.isFailure(exit)) {
                ...
                expect(cause._tag).toBe("Fail");
                expect(cause.error._tag).toBe("ProviderNotFoundError");
                expect(cause.error.providerName).toBe("");
            }
        });
        it("should fail with ProviderNotFoundError for case-sensitive provider names", async () => {
            ...
            if (Exit.isFailure(exit)) {
                ...
                expect(cause._tag).toBe("Fail");
                expect(cause.error._tag).toBe("ProviderNotFoundError");
                expect(cause.error.providerName).toBe("openai");
            }
        });
    });
    describe("getProvider", () => {
        it("should retrieve OpenAI provider with correct properties", async () => {
            ...
            if (Exit.isSuccess(exit)) {
                ...
                expect(provider.supportedModels).toEqual(expect.arrayContaining([
                    "gpt-4",
                    "gpt-3.5-turbo",
                    "gpt-5",
                    "gpt-5-mini",
        ...
        it("should return all available models", async () => {
            ...
            if (Exit.isSuccess(exit)) {
                ...
                for (const model of models) {
                    ...
                    expect(model.capabilities.every((cap) => typeof cap === "string")).toBe(true);
    ...
    describe("getModelsByCapability", () => {
        it("should return models with text-generation capability", async () => {
            ...
            if (Exit.isSuccess(exit)) {
                ...
                expect(models.every((model) => model.capabilities.includes("text-generation"))).toBe(true);
            }
        });
        it("should return models with code-generation capability", async () => {
            ...
            if (Exit.isSuccess(exit)) {
                const models = exit.value;
                expect(models.length).toBeGreaterThan(0);
                expect(models.every((model) => model.capabilities.includes("code-generation"))).toBe(true);
            }
        });
        ...
        it("should return empty array for non-existent capability", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByCapability("non-existent-capability");
            }));
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const models = exit.value;
    ...
    describe("getModelsByProviderAndCapability", () => {
        it("should return OpenAI models with code-generation capability", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByProviderAndCapability("OpenAI", "code-generation");
            }));
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const models = exit.value;
                ...
                expect(models.some((model) => model.name === "gpt-3.5-turbo")).toBe(true);
                expect(models.every((model) => model.capabilities.includes("code-generation"))).toBe(true);
            }
        });
        it("should return Anthropic models with text-generation capability", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByProviderAndCapability("Anthropic", "text-generation");
            }));
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const models = exit.value;
                expect(models.length).toBe(7);
                expect(models.some((model) => model.name === "claude-opus-4-1-20250805")).toBe(true);
                expect(models.some((model) => model.name === "claude-sonnet-4-20250514")).toBe(true);
                expect(models.every((model) => model.capabilities.includes("text-generation"))).toBe(true);
            }
        });
        it("should return only models with specific capability from provider", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByProviderAndCapability("Anthropic", "image-analysis");
            }));
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const models = exit.value;
        ...
        it("should return empty array when provider has no models with capability", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByProviderAndCapability("OpenAI", "image-analysis");
            }));
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const models = exit.value;
        ...
        it("should fail with ProviderNotFoundError for non-existent provider", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByProviderAndCapability("NonExistentProvider", "text-generation");
            }));
            expect(Exit.isFailure(exit)).toBe(true);
            if (Exit.isFailure(exit)) {
                const cause = exit.cause;
        ...
        it("should fail with ProviderNotFoundError for empty provider name", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                const service = yield* ModelService;
                return yield* service.getModelsByProviderAndCapability("", "text-generation");
            }));
            expect(Exit.isFailure(exit)).toBe(true);
            if (Exit.isFailure(exit)) {
                const cause = exit.cause;
    ...
    describe("Edge Cases and Error Handling", () => {
        it("should handle concurrent access to the same model", async () => {
            ...
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const [model1, model2] = exit.value;
        ...
        it("should handle concurrent access to different models", async () => {
            ...
            expect(Exit.isSuccess(exit)).toBe(true);
            if (Exit.isSuccess(exit)) {
                const [gpt4, gpt35, claude] = exit.value;
        ...
        it("should validate provider properties are consistent", async () => {
            const exit = await runEffect(Effect.gen(function* () {
                ...
                for (const providerName of providers) {
                    ...
                    expect(provider.supportedModels.every((model) => typeof model === "string")).toBe(true);
                }
                return providers;
            }));
            expect(Exit.isSuccess(exit)).toBe(true);
    ...
    describe("Integration Tests", () => {
        ...
        it("should find models by multiple capabilities", async () => {
            ...
            if (Exit.isSuccess(exit)) {
                ...
                const textGenResult = results.find((r) => r.capability === "text-generation");
                expect(textGenResult).toBeDefined();
                if (textGenResult) {
                    expect(textGenResult.modelCount).toBeGreaterThan(0);
                }
                const codeGenResult = results.find((r) => r.capability === "code-generation");
                expect(codeGenResult).toBeDefined();
                if (codeGenResult) {
                    expect(codeGenResult.modelCount).toBeGreaterThan(0);
                }
                const imageAnalysisResult = results.find((r) => r.capability === "image-analysis");
                expect(imageAnalysisResult).toBeDefined();
                if (imageAnalysisResult) {
                    expect(imageAnalysisResult.modelCount).toBe(3);
                    expect(imageAnalysisResult.models).toContain("claude-opus-4-1-20250805");
                }
            }
        });
    });
