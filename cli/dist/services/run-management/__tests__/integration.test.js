...
describe("RunManagement Integration", () => {
    it("should complete full run lifecycle", () => Effect.gen(function* () {
        // Create a new run
        const run = yield* RunManagementApi.createRun("test-run").pipe(Effect.provide(testLayer));
        ...
        const retrievedRun = yield* RunManagementApi.getRunInfo(run.name).pipe(Effect.provide(testLayer));
        expect(retrievedRun.name).toBe(run.name);
        expect(retrievedRun.directory).toBe(run.directory);
        // Test getting run directory
        const runDir = yield* RunManagementApi.getRunDirectory(run.name).pipe(Effect.provide(testLayer));
