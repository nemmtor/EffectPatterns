...
describe("RunManagement Service", () => {
    it("should create a run with sequential numbering", () => Effect.gen(function* () {
        const run1 = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
        const run2 = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
    ...
    it("should create a run with custom prefix", () => Effect.gen(function* () {
        const run = yield* RunManagementApi.createRun("test").pipe(Effect.provide(testLayer));
        expect(run.name).toContain("test_");
    }));
    it("should list runs in descending order", () => Effect.gen(function* () {
        yield* RunManagementApi.createRun("first").pipe(Effect.provide(testLayer));
        yield* RunManagementApi.createRun("second").pipe(Effect.provide(testLayer));
        const runs = yield* RunManagementApi.listRuns().pipe(Effect.provide(testLayer));
        expect(runs.length).toBeGreaterThanOrEqual(2);
        expect(runs[0].number).toBeGreaterThan(runs[1].number);
    }));
    it("should create run directory structure", () => Effect.gen(function* () {
        const run = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
    ...
    it("should persist run info", () => Effect.gen(function* () {
        const run = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
        const retrieved = yield* RunManagementApi.getRunInfo(run.name).pipe(Effect.provide(testLayer));
        expect(String(retrieved.name)).toBe(String(run.name));
        expect(Number(retrieved.number)).toBe(Number(run.number));
        expect(String(retrieved.directory)).toBe(String(run.directory));
    }));
    it("should handle state persistence across runs", () => Effect.gen(function* () {
        ...
        const run1 = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
        const run2 = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
        const run3 = yield* RunManagementApi.createRun().pipe(Effect.provide(testLayer));
