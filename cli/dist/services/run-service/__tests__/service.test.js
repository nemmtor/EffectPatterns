...
describe("RunService", () => {
    ...
    describe("Error Handling", () => {
        it("should fail when getting run path with no active run", () => Effect.gen(function* () {
            ...
            if (!currentRun) {
                const result = yield* runService.getRunPath().pipe(Effect.flip, Effect.map((error) => ({ hasError: true, errorMessage: error.message })));
        ...
        it("should fail when getting file path with no active run", () => Effect.gen(function* () {
            ...
            const currentRun = yield* runService.getCurrentRun();
            if (!currentRun) {
                const result = yield* runService.getRunFilePath("test.txt").pipe(Effect.flip, Effect.map((error) => ({ hasError: true, errorMessage: error.message })));
                expect(result.hasError).toBe(true);
            }
            else {
                // If there is a current run, we can't test this scenario
