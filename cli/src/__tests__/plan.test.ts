...
import {
  ...
  runTestEffect,
  runTestExit,
} from "../runtime/testing-runtime.js";
import { ConfigService } from "../services/config-service/service.js";
import { buildLlmExecutionPlanEffect } from "../services/llm-service/service.js";

function runCli(command: typeof planCommand, args: readonly string[]) {
...
describe("plan command", () => {
  ...
  it("plan list shows defaults when no overrides", async () => {
    ...
    const plan = await runTestEffect(
      buildLlmExecutionPlanEffect("google", "gemini-2.5-flash")
    );
  ...
  it("plan create sets fallbacks", async () => {
    ...
    const fallbacks = await runTestEffect(
      Effect.gen(function* () {
        const cfg = yield* ConfigService;
  ...
  it("plan reset behaves like clear and uses defaults", async () => {
    ...
    const plan = await runTestEffect(
      buildLlmExecutionPlanEffect("openai", "gpt-4o-mini")
    );
    expect(plan).toBeDefined();
  });
  ...
  it("plan create errors on invalid retries", async () => {
    const exit = await runTestExit(
      Command.run(planCommand, { name: "test", version: "0.0.0" })([
        "node",
        "test",
        "plan",
        "create",
