...
const authGet = Command.make(
  ...
  ({ provider }) =>
    Effect.gen(function* () {
      ...
      if (apiKey._tag === "Some") {
        yield* Console.log(
          `API key for ${provider}: ${apiKey.value.slice(
            0,
            4
...
const authList = Command.make("list", {}, () =>
  Effect.gen(function* () {
    const auth = yield* AuthService;
...
export const authCommand = Command.make("auth").pipe(
  Command.withSubcommands([authSet, authGet, authRemove, authList, authShow])
);
