...
const configGet = Command.make("get", { key: Args.text({ name: "key" }) }, ({ key }) =>
  Effect.gen(function*() {
...
const configSet = Command.make("set", { key: Args.text({ name: "key" }), value: Args.text({ name: "value" }) }, ({ key, value }) =>
  Effect.gen(function*() {
    const config = yield* ConfigService;
