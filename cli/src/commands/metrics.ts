...
const metricsReport = Command.make(
  "report",
  { format: formatOpt, output: outputOpt },
  ({ format, output }) =>
    Effect.gen(function* () {
      const metrics = yield* MetricsService;
      const resolvedFormat = Option.getOrElse(format, () => "console" as const);
...
const metricsLast = Command.make("last", {}, () =>
  Effect.gen(function* () {
    const metrics = yield* MetricsService;
...
const metricsClear = Command.make("clear", {}, () =>
  Effect.gen(function* () {
    const metrics = yield* MetricsService;
...
export const metricsCommand = Command.make("metrics").pipe(
  Command.withSubcommands([metricsReport, metricsLast, metricsClear])
);
