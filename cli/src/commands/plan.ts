...
const planCreate = Command.make(
  "create",
  { retries: retriesOpt, retryMs: retryMsOpt, fallbacks: fallbacksOpt },
  ({ retries, retryMs, fallbacks }) =>
    Effect.gen(function* () {
      const config = yield* ConfigService;

      if (
        Option.isNone(retries) &&
      ...
      if (Option.isSome(retries)) {
        ...
        if (!Number.isFinite(n) || n < 0) {
          yield* Effect.die(
            new Error("--retries must be a non-negative integer")
          );
        }
      ...
      if (Option.isSome(retryMs)) {
        ...
        if (!Number.isFinite(ms) || ms < 0) {
          yield* Effect.die(
            new Error(
              "--retry-ms must be a non-negative integer (milliseconds)"
            )
          );
        }
...
const planList = Command.make("list", {}, () =>
  Effect.gen(function* () {
    const config = yield* ConfigService;
...
const planClear = Command.make("clear", {}, () =>
  Effect.gen(function* () {
    const config = yield* ConfigService;
...
const planReset = Command.make("reset", {}, () =>
  Effect.gen(function* () {
    const config = yield* ConfigService;
    yield* config.remove("planRetries");
    yield* config.remove("planRetryMs");
    yield* config.remove("planFallbacks");
    yield* Console.log(
      "Reset to default execution plan (retries=1, retryMs=1000)"
    );
  })
);

export const planCommand = Command.make("plan").pipe(
  Command.withSubcommands([planCreate, planList, planClear, planReset])
);
