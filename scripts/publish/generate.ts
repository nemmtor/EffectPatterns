...
const generateReadme = ({
  ...
  Effect.gen(function* () {
    ...
    const patterns: PatternFrontmatter[] = yield* Effect.forEach(mdxFiles, (file) =>
      Effect.gen(function* () {
        const content = yield* fs.readFileString(path.join(indir, file));
        const parsed = yield* mdxService.readMdxAndFrontmatter(path.join(indir, file));
        return parsed.frontmatter as unknown as PatternFrontmatter;
      })
    );
    ...
    // Write README
    yield* fs.writeFileString(outfile, readmeContent);
    console.log("✅ README.md has been successfully generated!");
  });

// Run if called directly
if (require.main === module) {
  const program = generateReadme();
