 ...
 * qa-report.ts
 *
 * QA report generation script. Aggregates all QA results from content/qa/results/
 * and generates a comprehensive JSON report with statistics and analysis.
 *
 * Usage:
 *   bun run qa:report
 */
...
function generateRecommendations(results: any[], failedPatterns: any[]): string[] {
	const recommendations: string[] = [];

	if (failedPatterns.length > 0) {
		recommendations.push(`Found ${failedPatterns.length} failed patterns. Run "bun run qa:repair" to fix them.`);
	...
	if (results.length === 0) {
		recommendations.push("No QA results found. Run \"bun run qa:process\" to generate validation results.");
	}
...
function analyzeCommonIssues(failedPatterns: any[]): string[] {
	...
	for (const [type, count] of Object.entries(errorCounts)) {
		if (count > 1) {
			issues.push(`Multiple patterns have ${type} issues (${count} occurrences). Consider batch fixes.`);
		}
	}
...
function categorizeError(error: string): string {
	...
	if (errorLower.includes('import') || errorLower.includes('export')) return 'import/export issues';
	if (errorLower.includes('type') || errorLower.includes('typescript')) return 'TypeScript errors';
	if (errorLower.includes('deprecated') || errorLower.includes('outdated')) return 'deprecated APIs';
	if (errorLower.includes('example') || errorLower.includes('demo')) return 'example problems';
	if (errorLower.includes('documentation') || errorLower.includes('clarity')) return 'documentation issues';
	if (errorLower.includes('metadata') || errorLower.includes('frontmatter')) return 'metadata issues';
