...
async function showStatus() {
	...
	try {
		...
		if (qaResults.length === 0) {
			...
			console.log('Run "bun run qa:process" to generate QA results.');
			return;
		}
		...
		if (failed > 0) {
			...
			for (const [level, stats] of Object.entries(skillLevels)) {
				const levelRate = (stats.passed + stats.failed) > 0
					? (stats.passed / (stats.passed + stats.failed) * 100).toFixed(1)
					: 0;
				console.log(`  ${level}: ${stats.passed}/${stats.passed + stats.failed} (${levelRate}%)`);
			}
		}
...
function categorizeError(error: string): string {
	...
	if (errorLower.includes('import') || errorLower.includes('export')) return 'imports';
	if (errorLower.includes('type') || errorLower.includes('typescript')) return 'typescript';
	if (errorLower.includes('deprecated') || errorLower.includes('outdated')) return 'deprecated';
	if (errorLower.includes('example') || errorLower.includes('demo')) return 'examples';
	if (errorLower.includes('documentation') || errorLower.includes('clarity')) return 'documentation';
	if (errorLower.includes('metadata') || errorLower.includes('frontmatter')) return 'metadata';
	if (errorLower.includes('pattern') || errorLower.includes('best practice')) return 'patterns';
